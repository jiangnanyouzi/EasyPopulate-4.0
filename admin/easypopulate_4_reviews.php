...
while ($contents = fgetcsv($handle, 0, $csv_delimiter, $csv_enclosure)) { // while #1 - Main Loop
    ...
    $query = "SELECT * FROM " . TABLE_PRODUCTS . " WHERE (id = :v_id:) LIMIT 1";
    ...
    while ($row = ($ep_uses_mysqli ? mysqli_fetch_array($result) : mysql_fetch_array($result))) { // BEGIN while #2
        ...
        $sql = $db->bindVars($sql, ':v_customers_name:', $v_customers_name, 'string');
        ...
        $description_sql = $db->bindVars($description_sql, ':v_reviews_text:', '', 'string');
